...
footprint the KV cache is already the biggest resident on the GPU — but the 2x/4x
quantization claim applies to **dense-KV models only**. Our LFM2.5-2.6B is a hybrid
(dense + Mamba) where KV lives on just 8 of 30 layers, so the 8-bit ceiling here is
~53% of resident cached state — quantizing KV is **not** the cheapest lever on this
model (see the LFM2.5-2.6B section below).
...
and it holds **4x**. The tokens-in-budget math below assumes **dense K/V per layer** —
it does not model the LFM hybrid's 8-of-30-layer KV split.
...
to feed it calibrated scales. Measure: `sglang:kv_cache_memory_usage_gb` and
`sglang:cache_hit_rate`. Baselines:
...
- Our LFM2.5-2.6B is a small hybrid — the perfect worst case for 4-bit. **On this
  hybrid, keep fp16 KV; quantized fp8 is not a beneficial lever** without explicit
  model/scale-path support. (On dense-KV models the 8-bit 2x gain is real if QA
  passes.)
- Squeeze order on a fixed 12 GiB card (dense-KV models): quantize KV (8-bit) > trim
  context > drop a seat. On the LFM hybrid, KV quantization isn't a top lever, so the
  order shifts to trim context / drop a seat.
